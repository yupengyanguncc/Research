...
for i, (k, v, c, idx, hv) in enumerate(zip(keys, values, colors, initial_indices, hash_values)):
    probe_count = 0
    current_idx = (hv + probe_count * probe_count) % bucket_count
    ...
    while table[current_idx] is not None:
        ...
        calc = f"(hash({k}) + {probe_count}*{probe_count}) % {bucket_count} = ({hv} + {probe_count}*{probe_count}) % {bucket_count} = {current_idx}"
...
plt.savefig('assets/image/hash_table_quadratic_probing.png', bbox_inches='tight', dpi=300)
plt.close() 